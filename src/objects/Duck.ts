...
export class Duck extends Phaser.Physics.Arcade.Sprite {
  ...
  private duckSize: number = 50;
  private movementSpeed: number = 250;
  private riverBounds: { left: number; right: number } = { left: 60, right: GAME_WIDTH - 60 };
  ...
  constructor(scene: Phaser.Scene, x: number, y: number) {
    ...
    if (this.body) {
      ...
      this.body.setSize(this.duckSize * 0.8, this.duckSize * 0.8);
      this.body.setOffset(this.duckSize * 0.1, this.duckSize * 0.1);
    }
